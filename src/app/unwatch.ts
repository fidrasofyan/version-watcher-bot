...
export async function unwatch(req: TelegramRequest): Promise<TelegramResponse> {
  const watchList = await kysely
    .selectFrom("watch_list as wl")
    .innerJoin("product", "product.id", "wl.product_id")
    .select(["product.name as product_name"])
    .where("wl.chat_id", "=", req.message!.chat.id.toString())
  ...
  if (watchList.length === 0) {
    ...
    text += watchList
      .map((p) => `• ${p.product_name.toUpperCase()} - /unwatch_${p.product_name.replaceAll("-", "_")}`)
      .join("\n");
  }
