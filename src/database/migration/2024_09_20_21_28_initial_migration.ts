...
export async function up(db: Kysely<any>): Promise<void> {
  ...
  await db.schema
    .createTable("chat")
    .addColumn("id", "bigint", (col) => col.primaryKey())
    .addColumn("command", "varchar(100)", (col) => col.notNull())
  ...
  await db.schema
    ...
    .addColumn("url", "varchar(2048)", (col) => col.unique().notNull())
  ...
  await db.schema
    ...
    .addColumn("id", "bigserial", (col) => col.primaryKey())
    .addColumn("product_id", "bigint", (col) => col.notNull())
  ...
  await db.schema
    .createIndex("product_version_version_product_id_unique_index")
    .on("product_version")
    .unique()
  ...
  await db.schema
    .createIndex("watch_list_chat_id_product_id_unique_index")
    .unique()
    .on("watch_list")
...
export async function down(db: Kysely<any>): Promise<void> {
  await db.schema.dropTable("watch_list").ifExists().execute();
  await db.schema.dropTable("product_version").ifExists().execute();
